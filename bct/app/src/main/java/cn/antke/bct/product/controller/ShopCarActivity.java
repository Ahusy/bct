...
public class ShopCarActivity extends ToolBarActivity implements View.OnClickListener {
		...
		if (CommonConstant.REQUEST_NET_SUCCESS.equals(result.getResultCode())) {
			switch (requestCode) {
				case CommonConstant.REQUEST_NET_ONE:{
					...
					for (ShopCarEntity sce:shopCarEntities) {
						sce.setChecked(true);
						List<ProductDetailEntity> productDetailEntities = sce.getProductDetailEntities();
						for (ProductDetailEntity pde : productDetailEntities) {
							pde.setChecked(true);
						}
					}
	...
	public void updateCount() {
		...
		tvShopCarAmount.setText(Html.fromHtml("合计:"+InputUtil.formatPrice(integral+"", price+"")));
