...
public class ProductListActivity extends ToolBarActivity implements View.OnClickListener {
	...
	@ViewInject(R.id.fl_product_sort_zh)
	private View flProductSortZh;
	@ViewInject(R.id.tv_product_sort_zh)
	private TextView tvProductSortZh;
	...
	private void initView() {
		...
		setLeftTitle(title);
//		tvProductSortZh.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.product_down_green_icon, 0);
		tvProductSortZh.setSelected(true);
		isHightToLow = true;
		preSort = tvProductSortZh;
		...
		flProductSortZh.setOnClickListener(this);
	}
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.fl_product_sort_zh:
				isZH=true;
				sort(tvProductSortZh);
				break;
		}
	}

	private void sort(TextView textView) {
		if (preSort != textView&&preSort!=tvProductSortZh) {
		...
		if(!isZH){
			tvProductSortZh.setSelected(false);
			if (isHightToLow) {
